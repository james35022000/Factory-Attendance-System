...
<div class="container">
  ...
  <div class="row">
	...
	<?php require_once('db.php');
		$ID = $_POST['BussTripID'];
		$DATE = $_POST['BussTripDate'];
		$REASON = $_POST['BussTripReason'];
		if(mysql_query("UPDATE `canwall_test`.`Attendance` SET `Absence` = '出差', `Reason` = '$REASON', `Approval` = '1' WHERE `Attendance`.`ID` = '$ID' AND `Attendance`.`Date` = '$DATE';")) {  ?>
			<div class="col-lg-4" style="font-size:150%">指定出差完成</div>
	<?php 
		}  
		else { ?>
			<div class="col-lg-4" style="font-size:150%">指定出差失敗，請確認資訊後再試</div>
	<?php 
		} ?>
		<button class="btn btn-sm btn-danger" onclick="javascript:location.href='home.php'">返回</button>
    </div>
  </div>
</div>
