...
<div class="container">
  ...
  <div class="row">
	...
	<?php require_once('db.php');
		$ID = $_GET['ID'];
		$DATE = $_GET['DATE'];
		if(mysql_query("UPDATE `canwall_test`.`Attendance` SET `Absence` = NULL, `Reason` = NULL, `Approval` = '0' WHERE `Attendance`.`ID` = '$ID' AND `Attendance`.`Date` = '$DATE';")) {  ?>
			<div class="col-lg-4" style="font-size:150%">已刪除</div>
	<?php 
		}  
		else { ?>
			<div class="col-lg-4" style="font-size:150%">刪除失敗，請稍後再試</div>
	<?php 
		} ?>
		<button class="btn btn-sm btn-danger" onclick="javascript:location.href='approvalBrk.php'">返回</button>
    </div>
  </div>
</div>
