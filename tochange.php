        ...
        <div class="col-lg-10">
          <!--區塊功能打在這-->	
			<?php require_once('db.php');
				...
				$query = mysql_query("SELECT * FROM `test` WHERE `id` = '$cid'");
				$data=mysql_fetch_assoc($query);
				
				if (!$data){?>
					<div class="row"><font size="5"><b>查無此員工id，請確認後再試一次</b></font></div>
                    <div class="row">輸入的員工id : <?php print $cid; ?></div>
			...
			<?php }else{ 
			$sql = "UPDATE `test` SET `prior` = '$cpri' WHERE `id`='$cid'";
			//print $sql;
					if(mysql_query($sql)){ ?>
						<div class="row"><font size="5"><b>更新成功</b></font></div>
							
							<div class="row">員工id : <?php print $cid; ?></div>
							<div class="row">更新員工權限 : <?php if($cpri==0){print '員工';
																}else if($cpri==1){print '領班';
																}else{print '主任';};?>
						</div>
						<br>
			...
			<?php }else{ ?>
                            ...
                            <div class="row">輸入id : <?php print $cid; ?></div>
							<form id="tochange" action="list.php" method="post">
							<input name="送出" type="submit" class="btn btn-sm btn-primary" id="back" value="返回員工名單"/>
				  　 		</form>
